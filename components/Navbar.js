...
import { useSession, signOut } from "next-auth/react"; // Import useSession to manage session state
import { useCartStore } from "@/stores/useCartStore"; // Assuming you still want to use cart store

const Navbar = () => {
  const { data: session, status } = useSession(); // Use useSession hook to get session data
  const { cart } = useCartStore(); // Cart state from Zustand store (if needed)
