  ...
  useEffect(() => {
    const fetchAnalyticsData = async () => {
      try {
        ...
        setAnalyticsData(response.data); // assuming API returns all in root
        setDailySalesData(response.data.dailySalesData || []);
        console.log("✅ Daily Sales Data:", response.data.dailySalesData); // 👈 LOG HERE
      } catch (error) {
      ...
      >
        <ResponsiveContainer width="100%" height={400}>
          <LineChart data={dailySalesData}>
            ...
            {/* ✅ corrected to match dailySalesData keys */}
